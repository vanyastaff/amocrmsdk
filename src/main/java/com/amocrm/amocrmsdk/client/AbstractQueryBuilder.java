...
import com.amocrm.amocrmsdk.queries.EnumParam;
import com.amocrm.amocrmsdk.utils.URLBuilder;
...
public abstract class AbstractQueryBuilder<T, R> extends ApiRequest<R> {
     ...
     */
    public AbstractQueryBuilder(AmoApiClient client, String endpoint, Type type) {
        super(client.getApiEndpoint() + endpoint, client.getTransportClient(), client.getGson(), client.getRetryAttemptsInternalServerErrorCount(), type);
    }
     ...
     * @return string query
     */
    protected static String mapToGetString(Map<String, Object> params) {
        return URLBuilder.httpBuildQuery(params, "utf-8");
    }
    ...
    public T unsafeParam(String key, Map<String, Object> value) {
        params.put(key, value);
        return getThis();
    }
    ...
    protected T setAuth(Auth auth) {
        if(auth == null) throw new IllegalArgumentException("Authorization object does not set");
        auth.build(this);
        return getThis();
    }
    ...
    public Map<String, Object> build() {
        if (essentialKeys() != null && !params.keySet().containsAll(essentialKeys())) {
